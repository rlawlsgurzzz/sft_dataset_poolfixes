...
- cycle 구간의 command_text는 source command_text의 문장 구조, 동사, 어미, 조사, 말투를 재사용한다. 단, cycle source command_text 안에 actor 또는 target unitId가 명시되어 있으면, cycle output command_text에서는 명시된 actor/target unitId를 모두 source와 다른 valid unitId로 재매핑한다. 
- 아군 unitId는 다른 아군 unitId로, 적 unitId는 다른 적 unitId로 바꾼다. unitId를 재매핑하더라도 selected_bucket의 actor_selection, target_selection, action 의미, edge_flags, actor/target 구조와 개수는 유지한다. 
- command_text, command_spec.slots, area_situation, gold, output은 모두 재매핑된 unitId와 일관되어야 한다.

battlefield_diversity_hint 규칙:
- cycle_reuse_plan_1_based item에 battlefield_diversity_hint가 있으면, 해당 output_index_1_based sample의 area_situation을 만들 때 반영한다.
- battlefield_diversity_hint는 [unitId, isAlive, hpRatio, attackRatioToAvg] 4개 값을 한 묶음으로 반복하는 flat array다.
- 각 묶음은 "{unitId}는 {isAlive}고, hpRatio가 {hpRatio}이며, attackRatioToAvg가 {attackRatioToAvg}이다."라는 뜻이다.
- battlefield_diversity_hint는 해당 unitId의 isAlive, hpRatio, attackRatioToAvg만 유도한다.
- isAlive=false인 hint unit은 hpRatio=0.0으로 만든다.
...
- cycle 구간의 command_text는 paraphrase 대상이 아니며, 현재 item의 cycle_reuse_plan_1_based가 지정한 source command_text의 표현 형식을 재사용한다. 
- cycle source에 actor 또는 target unitId가 명시되어 있으면, cycle output command_text는 그 actor/target unitId를 모두 source와 다른 valid unitId로 재매핑한다. 아군은 다른 아군으로, 적은 다른 적으로 바꾸며, actor/target 구조와 개수는 유지한다. 
- command_text, area_situation, gold, output은 새로 만들고, 재매핑된 unitId와 일관되게 작성한다.
